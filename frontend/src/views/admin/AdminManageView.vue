  ...
  <section class="crud-page">
    <div class="crud-card search-card">
      <div class="crud-card__header">
        <div>
          <p class="page-kicker">Admin Management</p>
          <h3>管理员管理</h3>
        </div>
      ...
      <el-form :model="searchForm" inline label-width="80px" class="search-form">
        ...
        <el-form-item label="角色">
          <el-select v-model="searchForm.roleName" placeholder="全部角色" clearable style="width: 160px">
      ...
      <el-table v-loading="loading" :data="tableData" border class="crud-table">
        ...
        <el-table-column label="角色" width="140">
          <template #default="scope">
            <el-tag :type="scope.row.roleName === 'SUPER_ADMIN' ? 'danger' : 'success'">{{ roleLabel(scope.row.roleName) }}</el-tag>
          </template>
        </el-table-column>
...
import { createAdmin, deleteAdmin, fetchAdminDetail, fetchAdminPage, updateAdmin, updateAdminStatus } from '@/api/adminManage'
...
function roleLabel(roleName) {
  return roleOptions.find((item) => item.value === roleName)?.label || roleName || '-'
}
...
async function openEdit(id) {
  try {
    const { data } = await fetchAdminDetail(id)
    dialog.visible = true
    dialog.mode = 'edit'
    dialog.id = id
    Object.assign(form, {
...
async function handleSubmit() {
  ...
  try {
    if (dialog.mode === 'create') {
      ...
      await updateAdmin(dialog.id, form)
